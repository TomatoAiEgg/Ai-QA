...
public class ConversationRepository {
    ...
    public void updateTitleIfDefault(UUID id, String newTitle) {
        jdbcTemplate.update(
            "UPDATE conversations SET title = ?, updated_at = NOW() WHERE id = ? AND (title IS NULL OR title = '' OR title = '新的对话' OR title = 'New Chat')",
            newTitle, id
        );
    }
    ...
    public List<MessageRow> listMessages(UUID conversationId, int limit) {
        return jdbcTemplate.query(
            "SELECT id, conversation_id, role, content, created_at FROM conversation_messages WHERE conversation_id = ? ORDER BY created_at ASC LIMIT ?",
    ...
    public List<MessageRow> listRecentMessages(UUID conversationId, int limit) {
        List<MessageRow> items = jdbcTemplate.query(
            "SELECT id, conversation_id, role, content, created_at FROM conversation_messages WHERE conversation_id = ? ORDER BY created_at DESC LIMIT ?",
            messageRowMapper, conversationId, limit
        );
